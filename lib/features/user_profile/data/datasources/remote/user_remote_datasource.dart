...
class UserFirestoreImpl implements UserInfoRemoteDatasource{
    ...
    return await _prvCollectionRef()
      ...
      .then((value) {
        ...
        try {
          ...
          dekhao('fetchCurrentUserInfo failed!. error ${e.toString()}');
  ...
  Future<void> saveUserInfo({required UserPrvModel userPrv, required UserPubModel userPub}) async{
    ...
    batch.set(_prvCollectionRef().doc(userPrv.docId), userPrv.toMap());
    batch.set(_pubCollectionRef().doc(userPrv.docId), userPub.toMap());
    await batch.commit();
  }
